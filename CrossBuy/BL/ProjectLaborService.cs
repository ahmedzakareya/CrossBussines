	...
	public class ProjectLaborService : IProjectLaborService
	{
		...
		private async Task<(decimal hours, decimal cost, string? err)> TaskCostAsync(int companyId, int taskId, bool validateRate)
		{
			...
			foreach (var b in byEmp)
			{
				...
				if (validateRate && rate <= 0) return (0, 0, $"الموظف #{b.emp} بلا سعر ساعة قابل للاشتقاق — اضبطه أولًا");
		...
		public async Task<(bool ok, string? error)> PostTaskLaborAsync(int companyId, int taskId, int? userId)
		{
			...
			if (task.EntityType != "Project" || !(task.EntityId > 0)) return (false, "المهمة غير مربوطة بمشروع");
			if (task.LaborPostedAt != null) return (false, "سبق ترحيل عمالة هذه المهمة");   // post-once
			int projectId = task.EntityId!.Value;

			var (hours, cost, err) = await TaskCostAsync(companyId, taskId, true);
			if (err != null) return (false, err);
			if (hours <= 0) return (false, "لا ساعات مسجّلة على المهمة لترحيلها");
			if (cost <= 0) return (false, "تكلفة العمالة صفر");
			...
			if (salAcc == null) return (false, $"حساب الرواتب ({SalaryAccountCode}) غير مُهيّأ");
			...
			if (cc == null) return (false, "حساب الرواتب يتطلب مركز تكلفة ولا يوجد مركز تكلفة معرّف");
			...
			var input = new JournalEntryInput
			{
				...
				Lines = new List<JournalLineInput>
				{
					...
					new() { AccountId = salAcc.Value, Debit = 0, Credit = cost, ProjectId = null, CostCenterId = cc, Description = "إعادة تصنيف رواتب إلى المشروع" },
				}
			};
