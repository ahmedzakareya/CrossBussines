...
namespace CrossBuy.BL
{
	...
	public class OpeningBalanceService : IOpeningBalanceService
	{
		...
		private async Task<string?> GuardAsync(int companyId)
		{
			...
			return c.Finalized ? "The opening balances have been locked — nothing can be added or edited" : null;
		}
		...
		public async Task<(bool ok, string? error, decimal total)> PostStockAsync(int companyId, DateTime cutoff, List<OpeningStockLineInput> lines, string? userId)
		{
			...
			foreach (var l in (lines ?? new()))
				if (await ExistsAsync(companyId, "Stock", $"item:{l.ItemId}/wh:{l.WarehouseId}"))
					return (false, $"Item {l.ItemId} in warehouse {l.WarehouseId} already has an opening balance entered", 0);
			var (ok, err, jeId, total) = await _stock.PostOpeningStockAsync(companyId, cutoff, lines!, userId);
			if (!ok) return (false, err, 0);
			foreach (var l in lines!) await LogAsync(companyId, "Stock", $"item:{l.ItemId}/wh:{l.WarehouseId}", "Opening stock", l.Qty * l.UnitCost, jeId, cutoff, userId);
		...
		public async Task<(bool ok, string? error)> PostArAsync(int companyId, DateTime cutoff, int customerId, decimal amount, string? userId)
		{
			...
			if (await ExistsAsync(companyId, "AR", $"cust:{customerId}")) return (false, "This customer already has an opening balance entered");
			...
			var obe = await AccId(companyId, "3301"); if (obe == null) return (false, "The opening-balance account (3301) does not exist");

			var inv = new SalesInvoice { CompanyID = companyId, CustomerId = customerId, InvoiceDate = cutoff.Date, SubTotal = R(amount), TaxTotal = 0, GrandTotal = R(amount), Status = "Posted", Notes = "Opening balance", CreatedAt = DateTime.UtcNow };
			_db.SalesInvoices.Add(inv); await _db.SaveChangesAsync();
			inv.InvoiceNo = $"OB-AR-{inv.ID:D5}";
			inv.Lines.Add(new SalesInvoiceLine { SalesInvoiceId = inv.ID, LineNo = 1, ItemDescription = "Opening balance", Qty = 1, UnitPrice = R(amount), RevenueAccountId = obe.Value, LineTotal = R(amount) });
			await _db.SaveChangesAsync();

			var lines = new List<JournalLineInput>
			{
				new() { AccountId = cust.ControlAccountId, Debit = R(amount), Credit = 0, Description = $"Opening balance for customer {cust.Name}" },
				new() { AccountId = obe.Value, Debit = 0, Credit = R(amount), Description = "Opening balance - customers" },
			};
			await FixCcAsync(companyId, lines);
			var (ok, err, je) = await _journals.CreateAndPostAsync(new JournalEntryInput { CompanyID = companyId, EntryDate = cutoff, JournalType = "Opening", SourceType = "OpeningARBalance", SourceId = inv.ID, Description = $"Opening balance for customer {cust.Name}", Lines = lines }, null);
			...
			await LogAsync(companyId, "AR", $"cust:{customerId}", $"Opening balance for customer {cust.Name}", amount, je.ID, cutoff, userId);
			return (true, null);
		}
		...
		public async Task<(bool ok, string? error)> PostApAsync(int companyId, DateTime cutoff, int vendorId, decimal amount, string? userId)
		{
			var g = await GuardAsync(companyId); if (g != null) return (false, g);
			if (amount <= 0) return (false, "The amount must be greater than zero");
			if (await ExistsAsync(companyId, "AP", $"vend:{vendorId}")) return (false, "This supplier already has an opening balance entered");
			...
			var inv = new PurchaseInvoice { CompanyID = companyId, VendorId = vendorId, InvoiceDate = cutoff.Date, SubTotal = R(amount), TaxTotal = 0, GrandTotal = R(amount), Status = "Posted", Notes = "Opening balance", CreatedAt = DateTime.UtcNow };
			_db.PurchaseInvoices.Add(inv); await _db.SaveChangesAsync();
			inv.InvoiceNo = $"OB-AP-{inv.ID:D5}";
			inv.Lines.Add(new PurchaseInvoiceLine { PurchaseInvoiceId = inv.ID, LineNo = 1, ItemDescription = "Opening balance", Qty = 1, UnitPrice = R(amount), ExpenseAccountId = obe.Value, LineTotal = R(amount) });
			await _db.SaveChangesAsync();

			var lines = new List<JournalLineInput>
			{
				new() { AccountId = obe.Value, Debit = R(amount), Credit = 0, Description = "Opening balance - suppliers" },
				new() { AccountId = ven.ControlAccountId, Debit = 0, Credit = R(amount), Description = $"Opening balance for supplier {ven.Name}" },
			};
			await FixCcAsync(companyId, lines);
			var (ok, err, je) = await _journals.CreateAndPostAsync(new JournalEntryInput { CompanyID = companyId, EntryDate = cutoff, JournalType = "Opening", SourceType = "OpeningAPBalance", SourceId = inv.ID, Description = $"Opening balance for supplier {ven.Name}", Lines = lines }, null);
			if (!ok) { _db.PurchaseInvoices.Remove(inv); await _db.SaveChangesAsync(); return (false, err); }
			inv.JournalEntryId = je!.ID; await _db.SaveChangesAsync();
			await LogAsync(companyId, "AP", $"vend:{vendorId}", $"Opening balance for supplier {ven.Name}", amount, je.ID, cutoff, userId);
			return (true, null);
		}
		...
		public async Task<(bool ok, string? error)> PostAssetAsync(int companyId, DateTime cutoff, FixedAssetInput input, decimal openingAccumDep, string? userId)
		{
			...
			await LogAsync(companyId, "Asset", $"asset:{asset!.ID}", $"Opening asset {asset.Name}", asset.Cost, asset.AcquisitionJournalEntryId, cutoff, userId);
			return (true, null);
		}
		...
		public async Task<(bool ok, string? error)> PostGlAsync(int companyId, DateTime cutoff, List<OpeningGlLineInput> lines, string? userId)
		{
			...
			if (lines.Count == 0) return (false, "Add at least one line");
			var obe = await AccId(companyId, "3301"); if (obe == null) return (false, "The opening-balance account (3301) does not exist");
			...
			foreach (var l in lines)
			{
				...
				if (await ExistsAsync(companyId, "GL", $"acc:{code}")) return (false, $"Account {code} already has an opening balance entered");
			}

			var jlines = lines.Select(l => new JournalLineInput { AccountId = l.AccountId, Debit = R(l.Debit), Credit = R(l.Credit), CostCenterId = l.CostCenterId, Description = "Opening balance" }).ToList();
			decimal net = R(jlines.Sum(l => l.Debit) - jlines.Sum(l => l.Credit));   // OBE absorbs the residual
			if (net > 0) jlines.Add(new JournalLineInput { AccountId = obe.Value, Debit = 0, Credit = net, Description = "Opening balance - adjustment" });
			else if (net < 0) jlines.Add(new JournalLineInput { AccountId = obe.Value, Debit = -net, Credit = 0, Description = "Opening balance - adjustment" });
			await FixCcAsync(companyId, jlines);

			var (ok, err, je) = await _journals.CreateAndPostAsync(new JournalEntryInput { CompanyID = companyId, EntryDate = cutoff, JournalType = "Opening", SourceType = "OpeningGL", SourceId = 0, Description = "Opening trial balance", Lines = jlines }, null);
			if (!ok) return (false, err);
			foreach (var l in lines)
			{
				var code = codes.GetValueOrDefault(l.AccountId, l.AccountId.ToString());
				await LogAsync(companyId, "GL", $"acc:{code}", $"Opening balance for account {code}", l.Debit - l.Credit, je!.ID, cutoff, userId);
			}
			return (true, null);
		}
		...
		{
			var c = await GetControlAsync(companyId);
			if (c.Finalized) return (false, "The opening balances have already been locked");
			var checks = await VerifyAsync(companyId);
			if (checks.Any(x => !x.Ok)) return (false, "Cannot lock — there are mismatched invariants; review the verification report");
